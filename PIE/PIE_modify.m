function im_out = PIE_modify( im_target,im_source,im_mask,m,c )
...
for i=1:c %for each color channel
    ...
    if m==1  % mixing gradients
        ...
        % @Thuan add
        im_source_1d = im_source(:);
        obj_thresh = (min(im_source_1d)+max(im_source_1d))/2;
        ...
        g_x_final(abs(g_mag_target)>abs(g_mag_source) & (im_source_1d<obj_thresh))=...
            g_x_target((g_mag_target>g_mag_source) & (im_source_1d<obj_thresh)); % @Thuan modified
        g_y_final(abs(g_mag_target)>abs(g_mag_source) & (im_source_1d<obj_thresh))=...
            g_y_target((g_mag_target>g_mag_source) & (im_source_1d<obj_thresh));  % @Thuan modified
